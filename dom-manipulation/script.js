...
/* ---------- “Server” (Mock API) ---------- */
/* We’ll simulate server data using JSONPlaceholder posts.
   Mapping:
   - post.body   -> quote.text
   - post.title  -> quote.category
   - post.id     -> quote.serverId (string)
*/
...
function saveLastQuoteIndex(index) {
  sessionStorage.setItem(SS_LAST_QUOTE_INDEX_KEY, String(index));
}
function getLastQuoteIndex() {
  const v = sessionStorage.getItem(SS_LAST_QUOTE_INDEX_KEY);
  return v == null ? null : Number(v);
}
function saveLastCategory(category) {
  localStorage.setItem(LS_LAST_CATEGORY_KEY, category);
}
function getLastCategory() {
  return localStorage.getItem(LS_LAST_CATEGORY_KEY) || "all";
}
...
function populateCategories() {
  ...
  filter.value = filter.querySelector(`option[value="${lastCat}"]`) ? lastCat : "all";
}
...
function addQuote() {
  ...
  quotes.push({ id: makeId(), text, category, updatedAt: Date.now(), dirty: true }); // mark dirty to push
  saveQuotes();
  populateCategories(); // update filter if a new category arrived
  textEl.value = ""; catEl.value = "";
  filterQuotes(); // refresh list based on current filter
  ...
  reader.onload = e => {
    try {
      ...
      const incoming = Array.isArray(imported)
        ? imported
        : (Array.isArray(imported?.quotes) ? imported.quotes : null);
      ...
      // simple de-dup by text+category
...
/* ---------- Server Sync ---------- */
/* Strategy:
   - pull (GET): fetch posts, map to server quotes
   - merge: if serverId matches a local item and differs -> conflict => server wins but we log it
   - push (POST): local items with dirty=true will be posted (mock returns an id).
*/
async function fetchServerQuotes() {
  try {
    // limit to a subset to keep UI light
    ...
    const serverQuotes = posts.map(p => ({
      ...
      // JSONPlaceholder has no timestamp; simulate with a fixed epoch increment
      serverStamp: Date.now() // used only to make “newer” on each fetch for demo
    })).filter(q => q.text);
    return serverQuotes;
  } catch (e) {
...
function mergeServerQuotes(serverQuotes) {
  ...
  // map local by serverId (if present) or by text+category for naive match
  const byServerId = new Map();
  quotes.forEach(q => { if (q.serverId) byServerId.set(q.serverId, q); });

  serverQuotes.forEach(srv => {
    // if we already have a local bound to this server item
    ...
    if (local) {
      ...
      if (!shallowEqualQuote(local, newShape)) {
        // conflict if local was changed since last sync AND differs from server
        ...
        // identical; mark synced
        local.dirty = false;
      }
    } else {
      // New from server: add and mark clean
      quotes.push({
        id: makeId(),
...
async function pushLocalChanges() {
  ...
  for (const q of dirty) {
    try {
      // JSONPlaceholder won’t actually persist, but returns a mock object with id.
      const res = await fetch(SERVER_URL, {
        ...
        body: JSON.stringify({
          ...
          body: q.text,
          userId: 1,
          // include our own metadata (server won’t store, but good for real backends)
          clientUpdatedAt: q.updatedAt
        })
      });
      const data = await res.json();
      // Bind returned id as serverId for future merges
      if (data && data.id != null) {
        q.serverId = String(data.id);
      }
      q.dirty = false;
      pushed++;
    } catch (e) {
      console.warn("Failed to push quote:", q, e);
      // keep it dirty for next attempt
    }
  }
  saveQuotes();
  return { pushed };
